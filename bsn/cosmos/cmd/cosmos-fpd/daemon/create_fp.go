...
func runCommandCreateFP(ctx client.Context, cmd *cobra.Command, _ []string) error {
	...
	if fp.KeyName == "" {
		cfg, err := config.LoadConfig(ctx.HomeDir)
		if err != nil {
			return fmt.Errorf("failed to load config from %s: %w", config.CfgFile(ctx.HomeDir), err)
		}
	...
	res, err := grpcClient.CreateFinalityProvider(
		cmd.Context(),
		fp.KeyName,
		fp.ChainID,
		fp.EotsPK,
