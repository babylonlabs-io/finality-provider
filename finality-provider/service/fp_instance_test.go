...
func TestFinalityProviderInstance_AllowlistBlocking(t *testing.T) {
	...
	// Set up basic mocks needed for startup
	mockConsumerController.EXPECT().GetFpRandCommitContext().Return("").AnyTimes()
	mockConsumerController.EXPECT().GetFpFinVoteContext().Return("").AnyTimes()
	...
	t.Run("FP_Not_In_Allowlist_Should_Fail", func(t *testing.T) {
		...
		// Try to start the FP - should fail
	...
	t.Run("FP_In_Allowlist_Should_Start", func(t *testing.T) {
		...
		// Mock other required calls for successful startup
		mockConsumerController.EXPECT().QueryFinalityActivationBlockHeight(gomock.Any()).
			Return(randomStartingHeight, nil).AnyTimes()
		mockConsumerController.EXPECT().QueryLastPublicRandCommit(gomock.Any(), gomock.Any()).
	...
	t.Run("Allowlist_Query_Error_Should_Fail", func(t *testing.T) {
		...
		mockConsumerController.EXPECT().QueryFinalityProviderInAllowlist(gomock.Any(), gomock.Any()).
			Return(false, expectedErr).Times(1)

		_, fpIns, cleanUp := startFinalityProviderAppWithRegisteredFp(t, r, mockBabylonController, mockConsumerController, true, randomStartingHeight, testutil.TestPubRandNum)
		defer cleanUp()

		// Try to start the FP - should fail with allowlist error
		err := fpIns.Start(context.Background())
